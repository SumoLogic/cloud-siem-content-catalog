...
|changeTarget|modified_property_Group.DisplayName||
...
|targetUser_email|properties.targetResources.1.userPrincipalName||
|targetUser_username|properties.targetResources.1.userPrincipalName|This is a split field. More info to come in the catalog later...|
|timestamp|time|We expect the orginal record value of `time` is in the format `yyyy-MM-dd'T'HH:mm:ss.SSSSSSSZ`|
|user_email|properties.initiatedBy.user.userPrincipalName||
|user_userId|properties.initiatedBy.user.id||
|user_username|properties.initiatedBy.user.userPrincipalName|This is a split field. More info to come in the catalog later...|
