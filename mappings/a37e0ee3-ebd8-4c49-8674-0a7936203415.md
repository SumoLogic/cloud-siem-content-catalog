# [Mappings](README.md): GCP VPC Flows
...
|timestamp|message.data.timestamp|We expect the original record value of `message.data.timestamp` is in the format `yyyy-MM-dd'T'HH:mm:ss`|
