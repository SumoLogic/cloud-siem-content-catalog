# [Mappings](README.md): AWS Network Firewall Netflow
...
|timestamp|event.timestamp|We expect the original record value of `event.timestamp` is in the format `yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ`|
